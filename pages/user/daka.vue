<template>
	<view :class="{ 'calendar-content-active ': infoShow }" class="calendar-content">
		<view class="example-info">目前打卡是全权由教练负责，如果对打卡记录有任何疑问，可以与教练沟通</view>
		<view class="example-title">打卡记录</view>
		...
		<view v-if="showCalendar">
			...
			<view :class="{ 'calendar-active': infoShow }" class="calendar-box">
				...
				<view v-if="timeData.lunar" class="calendar-info">
					<view>已选日期 : {{ timeData.fulldate }}</view>
					...
					<view>是否打卡 : {{ timeData.clockinfo.have ? '是' : '否' }}</view>
					<view>打卡时间 : {{ timeData.clockinfo.time || '无' }}</view>
					<view>运动前体重 : {{ timeData.clockinfo.before_weight || '无' }}</view>
					<view>运动前体重 : {{ timeData.clockinfo.after_weight || '无' }}</view>
					<!-- <button @click="getDaka">打卡</button> -->
...
<script>
	...
	export default {
		...
		onLoad() {
			this.tuser = uni.getStorageSync('tuser_id');
			setTimeout(() => {
		...
		methods: {
			...
			change(e) {
				...
				uni.request({
					url:'https://ygjs.mfmeat.top/index.php/api/user/dakaSearch',
					data:{'tuser':this.tuser,'time':e.fulldate},
					method:'POST',
					dataType:'json',
					success: (res) => {
						if(res.data.code == 1)
						{
							this.timeData.clockinfo.have = res.data.daka;
							this.timeData.clockinfo.time = res.data.daka_time;
							this.timeData.clockinfo.before_weight = res.data.before_weight;
							this.timeData.clockinfo.after_weight = res.data.after_weight;
